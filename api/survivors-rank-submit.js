...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const key = rankKey(profile.stationName);
    const path = `${RANK_ROOT}/${key}`;
    const current = await dbFetch(path);
    const oldScore = cleanNumber(current && current.score);
    const finalScore = Math.max(oldScore, score);
    let updated = false;
    ...
    if (score > oldScore) {
      ...
      const payload = {
        ...
        createdAtIso: current && current.createdAtIso ? current.createdAtIso : new Date(now).toISOString(),
        updatedAt: now,
